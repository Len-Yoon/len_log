...
<script>
    ...
    function showExcelUploadMemoModal() {
        $("#inputFile2").val("");

        $('#excelUploadMemoModal #excelUpload').off().on('click', function () {
            if (!$("#inputFile2").val()) {
                swal({
                    title: "WARNING",
                    text: "엑셀 파일을 선택 해주세요.",
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        for (var i = 1; i < data.length; i++) {
                            ...
                            if (data[i].length == 0) {
                                swal("WARNING", "데이터가 존재하지 않습니다.", "warning");
                                break;
                            } else if (code == undefined || code == "") {
                                swal("WARNING", "상품코드가 없습니다.", "warning");
                                break;
                            } else if (codeList.length < 2 || codeList.length > 3) {
                                swal("WARNING", "상품코드를 확인해 주세요.", "warning");
                                break;
                            } else if (regExp.test(codeList[0])) {
                                swal("WARNING", "상품코드에 카테고리 코드가 포함되어 있습니다.", "warning");
                                break;
                            } else {
                                status = 'Y';
                            }
        ...
        $('#excelUploadMemoModal').modal({backdrop: 'static', keyboard: false});
    }
                    ...
                    success: function (rtn) {
                        if(rtn != -1) {
                            swal({
                                ...
                                function () {
                                    $('#excelUploadMemoModal').modal('hide');
                                    showMemoLog();
                                });
                        }
                    }
...
<div class="modal fade" id="excelUploadMemoModal" style="z-index: 9999999999; box-shadow: 0px 0px 100px 0px black;  overflow-x: hidden; overflow-y: auto;" role="dialog">
    ...
    <div class="modal-dialog" style="width: 90%; box-shadow: 0px 0px 100px 0px black;">
        <div class="modal-content">
            ...
            <div class="modal-header">
                ...
                <button type="button" class="topButton" style="float: left;" onclick="showExcelUploadMemoModal()">+ 다중 메모</button>
            </div>
